...
@test "memory_dir creates the dir under instance data root" {
  ...
  [[ "$result" == "$CLAUDIFY_INSTANCE_DIR/data/reminders" ]]
  [[ -d "$result" ]]
}
...
@test "memory_path returns <data-dir>/<filename>" {
  ...
  [[ "$result" == "$CLAUDIFY_INSTANCE_DIR/data/reminders/r.db" ]]
}

# ─── memory_export_env ────────────────────────────────────────────────────
@test "memory_export_env sets CLAUDIFY_SKILL_DATA to the skill's data dir" {
  memory_export_env reminders
  [[ "$CLAUDIFY_SKILL_DATA" == "$CLAUDIFY_INSTANCE_DIR/data/reminders" ]]
  [[ -d "$CLAUDIFY_SKILL_DATA" ]]
}
...
@test "manifest_set_skill with no memory args creates a bare entry" {
  ...
  result="$(jq -r '.skills | length' "$CLAUDIFY_INSTANCE_DIR/claudify.json")"
  [[ "$result" == "1" ]]
  result="$(jq -r '.skills[0].id' "$CLAUDIFY_INSTANCE_DIR/claudify.json")"
  [[ "$result" == "reminders" ]]
  # No memory key when neither slot was given
  result="$(jq -r '.skills[0] | has("memory")' "$CLAUDIFY_INSTANCE_DIR/claudify.json")"
  [[ "$result" == "false" ]]
}
...
@test "manifest_set_skill is idempotent on id (one entry per id)" {
  ...
  result="$(jq -r '.skills | length' "$CLAUDIFY_INSTANCE_DIR/claudify.json")"
  [[ "$result" == "1" ]]
  result="$(jq -r '.skills[0].memory.writes' "$CLAUDIFY_INSTANCE_DIR/claudify.json")"
  [[ "$result" == "reminders.db" ]]
}
