# Phase 4 — Security hardening (overview)

**Status:** ⏳ pending
**Estimated effort:** ~2.75 hr (across 6 sub-tasks)
**Depends on:** Phase 3 closure (3.5 backup-restore must ship; paths stable)
**Blocks:** Phase 5 (memory MCP — bot should be hardened before adding new attack surface)
...
- **Arm A — systemd hardening** (sub-tasks 4.1–4.6 below). Take
...
| # | Task | Effort | Tier |
|---|---|---|---|
| 1 | [4.1 — Tier-1 hardening (always-safe)](4.1-tier1-hardening.md) | 30 min | Critical |
| 2 | [4.2 — Filesystem write-restriction](4.2-fs-write-restriction.md) | 45 min | Critical |
| 3 | [4.3 — Address families + syscall filter](4.3-syscall-and-network.md) | 30 min | Critical |
| 4 | [4.4 — Tighten file permissions](4.4-file-permissions.md) | 15 min | Important |
| 5 | [4.5 — doctor.sh security section](4.5-doctor-security-section.md) | 30 min | Important |
| 6 | [4.6 — Security documentation](4.6-security-docs.md) | 15 min | Documentation |
...
- [ ] Service unit doesn't grant capabilities; runs as the user (covered by 4.1)
...
- [ ] systemd unit blocks non-IP socket families (covered by 4.3)
...
- **`bypassPermissions` stays.** It's the right call for an unattended bot. The fix for its risk is upstream isolation (this task), not turning permissions back on (which would break the bot). 4.6 documents the trade-off explicitly.
